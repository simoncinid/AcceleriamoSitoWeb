import { whatsappDisplay, whatsappNumber } from "@/lib/contact";
...
export function LegalIdentity() {
  return <dl className="legal-identity">
    ...
    {legalIdentity.address && <><dt>Sede</dt><dd>{legalIdentity.address}</dd></>}
    {legalIdentity.email && <><dt>Email di contatto e privacy</dt><dd><Link href={`mailto:${legalIdentity.email}`}>{legalIdentity.email}</Link></dd></>}
    <dt>Telefono e WhatsApp</dt><dd><a href={`tel:${whatsappNumber}`}>{whatsappDisplay}</a></dd>
  </dl>;
}
