...
La generazione prosegue per checkpoint tramite pagina aperta e, su Vercel, worker con `after()`. Il cron giornaliero riprende job ed email pendenti. Nessun documento incompleto viene pubblicato. Gli invii registrati non vengono ripetuti; un arresto tra invio SMTP e salvataggio può comunque produrre un duplicato.
